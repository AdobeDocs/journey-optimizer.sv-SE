...
#### Kontroll av nyttolastens storlek på resan {#journey-payload-size}

När du sparar eller publicerar en resa validerar Journey Optimizer den totala transportnyttolaststorleken för att bevara stabilitet och prestanda.

**Standardkonfiguration**

* **Maximal standardstorlek för begäran**: 2 MB (2 000 000 byte). Vissa organisationer kan ha anpassade begränsningar konfigurerade av Adobe.
* **Varningströskel**: 90 % av maxgränsen.
* **Feltröskel**: 100 % av maxgränsen. Det går inte att spara eller publicera och begäran returnerar **HTTP 413 Request Entity Too Large**.

**Scenarier för användarupplevelser**

* **Nyttolast &lt; 90 % av gränsen**: Resan sparar och publicerar utan fel. Inga varningar eller fel visas.
* **Nyttolast 90-99 % av gränsen**: Resan sparar och publicerar utan fel, med en varning om att optimera. Varningsmeddelande: **Varning**: Storleken på nyttolasten för resan ligger nära gränsen. Största nod: [NodeName] (typ: [NodeType], storlek: [N] byte).
* **Nyttolast >= 100 % av gränsen**: Det går inte att spara eller publicera på resa. Felmeddelande: **Fel**: Storleken på nyttolasten överskrider gränsen. Största nod: [NodeName] (typ: [NodeType], storlek: [N] byte).

**Felsvarsinformation**

Om begäran överskrider den tillåtna maxstorleken innehåller svaret **För stor begärandeentitet**. Transportnyttolasten överskrider den tillåtna maxstorleken. Granska felinformationen och optimera din resa.

**Felsökning och rekommendationer**

* Granska den största noden som markeras i varningen eller felmeddelandet.
* Förenkla villkoren, minska datamappningarna och ta bort onödiga steg och parametrar.
* Överväg att dela upp resan i mindre resor om det behövs.
* Om du tror att din organisation behöver en högre gräns kontaktar du Adobe.
...
* En profil kan inte finnas flera gånger under samma resa, samtidigt, för alla aktiva [versioner av resan](../building-journeys/publish-journey.md#journey-create-new-version). Om återinträde är aktiverat kan en profil återansluta en resa, men kan inte göra det förrän den tidigare instansen av resan har avslutats helt. [Läs mer](../building-journeys/end-journey.md)
...
* Journey Optimizer kräver att händelser direktuppspelas till datainsamlingens bastjänst (DCCS) för att kunna utlösa en resa. Händelser som är inkapslade i grupp, händelser som infogats via **frågetjänsten** eller händelser från interna Journey Optimizer-datauppsättningar (meddelandefeedback, e-postspårning osv.) kan inte användas för att utlösa en resa. I de fall där du inte kan få direktuppspelade händelser måste du skapa en målgrupp baserat på dessa händelser och använda aktiviteten **Läs målgrupp** i stället. Målgruppskvalificering kan användas tekniskt, men rekommenderas inte eftersom den kan orsaka efterföljande utmaningar baserat på de åtgärder som används.
...
Läs mer om resefrekvenser och dataflödesgränser i [det här avsnittet](../building-journeys/entry-management.md#journey-processing-rate).
...
* **[!UICONTROL Campaign Standard]**-aktiviteter kan inte användas med andra kanalaktiviteter: kort, kodbaserad upplevelse, e-post, push, SMS, meddelanden i appen, webb.
* **[!UICONTROL Campaign v7/v8]** aktiviteter kan användas tillsammans med interna kanalaktiviteter på samma resa.
...
* Aktiviteten i appen kan inte användas med **[!UICONTROL Campaign Standard]** aktiviteter.
