...
* Dokumentation för **förhandsgranskning av kodbaserade upplevelser med Experience Decisioning** klargör nu att **[!UICONTROL Simulate content]** endast är en förhandsgranskning av innehåll. Sammanhangsdata från live-begäranden från Edge simuleras inte vid redigering av förhandsgranskning. [Läs mer](../code-based/test-code-based.md#preview-code-based)

* Dokumentationen **Använd Adobe Experience Platform-data** har uppdaterats: skyddsritningarna anger inte längre att datasetsökningar inte kan kedjas, vilket återspeglar den aktuella produktfunktionen. [Läs mer](../data/lookup-aep-data.md)

* Aktivitetsdokumentationen för **Uppdatera profil** har uppdaterats till dokumentstöd för uppdatering av upp till fem profilattribut i en enda åtgärd. [Läs mer](../building-journeys/update-profiles.md)

* Dokumentationen **Läs målgrupp** och **Reseegenskaper** har uppdaterats för att förtydliga 91-dagars resans livscykel för återkommande resor. I schemaavsnittet bekräftas nu uttryckligen att återkommande resor utan slutdatum förblir Live de 91 senaste dagarna och att de globala Vanliga frågorna om tidsgränsen har utökats för att skilja mellan den 91-dagars profilen TTL och rapportfönstret för 91 dagar. [Läs mer](../building-journeys/read-audience.md#schedule)

* Aktivitetsdokumentationen för **datauppslagssökning** har uppdaterats för att klargöra att uppslagsnyckeln måste konfigureras i avancerat läge för att syntaxen `@datasetLookup{}` ska fungera i underordnade villkorsaktiviteter. Ett felsökningsavsnitt har lagts till med vägledning om hur du löser felet&quot;Det gick inte att hitta någon datauppsättning&quot;. [Läs mer](../building-journeys/dataset-lookup.md#troubleshooting)

* Dokumentationen för **Date Time-funktioner** har uppdaterats med ett nytt exempel som visar hur du formaterar en tidsstämpel från ett kontexthändelseattribut, inklusive `toDateTime()`-kravet, bakåttickssyntax för numeriska händelse-ID:n och en gemensam felförklaring för PQL-felet &quot;indata matchar inte&quot;. [Läs mer](../personalization/functions/dates.md#format-date)

* Dokumentationen för **samordnade kampanjer har uppdaterats för att visa att för filbaserad registrering av ändringsdata krävs fältet** och dess värden måste vara gemener **(upsert) eller** (delete), inte versaler, för filbaserad registrering av ändringsdata `_change_request_type` och `u`Kom igång med källkopplingar`d`. [Läs mer](../orchestrated/guardrails.md)

* Dokumentationen **Lägg till länkar och spårmeddelanden** har uppdaterats med vägledning om hur spårnings-ID (urlID) genereras: ett unikt urlID tilldelas bara när både URL:en och etiketten är unika. Om du vill spåra samma URL över flera e-postmeddelanden (eller flera gånger i ett e-postmeddelande) måste användarna använda en unik etikett för varje liknande URL. Annars kan [!DNL Journey Optimizer] inte avgöra vilken länk som klickades. [Läs mer](../email/message-tracking.md#track-across-multiple-emails)

* Dokumentationen **Skapa testprofiler** har uppdaterats med en viktig anteckning om identitetsbeskrivningskrav: när en datauppsättning tas bort och återskapas måste schemat behålla rätt identitetsbeskrivare i det primära identitetsfältet. Utan den flaggas inte kapslade profiler som `testProfile = true`, även om importen slutförs utan fel. En kontrollista för felsökning har lagts till. [Läs mer](../audience/creating-test-profiles.md)

* Aktivitetsdokumentationen **Läs målgrupp** har uppdaterats för att klargöra att en **affärshändelse**-aktivitet är ett undantag från regeln att Läs målgrupp måste vara den första aktiviteten i en resa. En anteckning har också lagts till som refererar till aktiviteten **Optimera** som ett avancerat alternativ för att styra målgruppsanpassning. [Läs mer](../building-journeys/read-audience.md)

* **Skicka med vågor** under resor är nu allmänt tillgängligt. Flaggan Begränsad tillgänglighet har tagits bort från dokumentationen. [Läs mer](../building-journeys/send-using-waves.md)

* Aktivitetsdokumentationen **Hoppa** har berikats med ett nytt designstrategiavsnitt - **Delresor i bitstorlek** - som förklarar hur komplexa slutflöden kan brytas till mindre, fokuserade delresor som är kopplade via hoppaktiviteten. [Läs mer](../building-journeys/jump.md#jump-strategy)

* Dokumentationen **Taggar** har uppdaterats med vägledning om hur du använder taggkategorier som ett alternativ till komplexa namnkonventioner. I ett nytt avsnitt förklaras hur du ställer in taggkategorier för skalbar resehantering. [Läs mer](../building-journeys/tags.md)

* Dokumentationen **Om datakällor** innehåller nu ett nytt avsnitt som hjälper dig att välja mellan tre dataåtkomststrategier: att få åtkomst till externa data via anpassade åtgärder, använda en datauppsättning som inte har aktiverats för profilen eller använda en profilaktiverad datauppsättning. Varje alternativ beskrivs med kompromisser och rekommenderade användningsområden. [Läs mer](../datasource/about-data-sources.md#data-access-strategy)

* Dokumentationen **Push Notification design** har uppdaterats med en anteckning som klargör hur universella länkar fungerar på iOS: om meddelande-URL:en är registrerad som en universell länk öppnas den associerade appen oavsett den valda webb-URL-åtgärden. Vägledning har lagts till om hur du tvingar en webbläsare att öppnas. [Läs mer](../push/design-push.md)

* En ny sida för **Övervaka dina AI-modeller** finns nu tillgänglig i beslutsdokumentationen. Det förklarar hur du spårar hälsa, utbildningsstatus och prestanda för personaliserade optimeringsmodeller direkt i [!DNL Journey Optimizer]. [Läs mer](../experience-decisioning/ranking/ai-model-observability.md)

* Den **avancerade HTML-redigeraren** (expertläge) för e-postmallar är nu tillgänglig med begränsad tillgänglighet. Dokumentationssidan är nu tillgänglig för alla. Med den här funktionen kan du visa och redigera HTML råformat för mallar för e-postinnehåll direkt från e-post-Designer. [Läs mer](../content-management/email-template-expert-mode.md)

* Dokumentationen för **URL-spårning** och **felsökning av resor** har uppdaterats för att dokumentera beteendet hos `context.system.source.actionId` under stängda resor. Stängda eller ej publicerade resor kan skapa tomma `{}` platshållare i spårnings-URL:er. Det har lagts till vägledning om hur du löser problemet genom att publicera om resan eller ta bort den berörda parametern. [Läs mer](../email/url-tracking.md)

* Dokumentationen för **Adobe Experience Platform-datakällan** har uppdaterats med ett meddelande om att endast enskilda profilbaserade XDM-scheman stöds i Data Source-konfigurationen. [Läs mer](../datasource/adobe-experience-platform-data-source.md)

* Dokumentationen **Datamängder som är TTL-skyddade** har förbättrats med en ny FAQ-post för att tydligt identifiera vilka datamängder som omfattas av TTL. TTL gäller endast för dataset i tidsserier - datamängder av posttyp som entitetsdatamängder, klassificeringsdatamängder och beslutsobjektsdatabaser omfattas inte av TTL och påverkas inte av skyddsutplaceringen. [Läs mer](../data/datasets-ttl.md)

* Dokumentationen för **reseegenskaperna** och **Pausa en resa** har uppdaterats för att dokumentera de nya paus- och återupptagningsfälten som nu finns tillgängliga i den tekniska informationen för resan. Knappen **Kopiera teknisk information** innehåller nu `lastPausedAt`, `lastPausedBy`, `lastPausedById`, `lastResumedAt`, `lastResumedBy` och `lastResumedById`, utöver det befintliga `pausedJourneySettings`-blocket. Ett nytt avsnitt har också lagts till på sidan **Pausa en resa** som förklarar hur du kan visa pausa och återuppta tidsstämplar direkt från reseegenskaperna. [Läs mer](../building-journeys/journey-properties.md)

## Februari 2026 {#february-2026}

* En ny sida finns nu tillgänglig för beslutshantering. Den listar alla operatorer, hjälpprogram och funktioner som stöds vid personalisering av erbjudandeinnehåll (representationer) med personaliseringsredigeraren. Använd den här listan för att undvika körningsfel. Endast dokumenterade funktioner stöds när du anpassar innehåll i Offer Decisioning. [Läs mer](../offers/offer-library/personalization-editor-supported-functions.md)

* **Skapa beslutsprinciper** och **Använd beslutsprinciper i meddelanden**-dokumentation har uppdaterats för e-post: en anteckning förklarar nu att när samma erbjudande kan väljas av fler än en beslutsprincip i e-postmeddelandetexten, dedupliceras erbjudanden (varje placering får ett annat erbjudande). Använd **Återanvänd beslutsutdata** om du vill visa samma erbjudande på flera platser (till exempel sidhuvud och sidfot). [Läs mer](../experience-decisioning/create-decision-policy.md)

* Sidan Beslutsobjekt har uppdaterats med information om push-kanal och anpassad händelsebegränsning. [Läs mer](../experience-decisioning/items.md#capping)

* Dokumentationen **Upplevelsehändelsesökning i resor** har uppdaterats med tidslinjen för borttagning: från och med 1 april 2026 har organisationer som inte har använt händelseattribut i reseuttryck under de senaste 90 dagarna inte längre tillgång till den här funktionen. Vanliga frågor och svar fokuserar nu på tidslinjen för återlösen och vem som påverkas, och Experience-händelsens schemasida har anpassats till en direkt länk till alternativa strategier. [Läs mer](../building-journeys/exp-event-lookup.md)

* Dokumentationen **Decisioning** har uppdaterats för **datauppslagssökning** med Adobe Experience Platform-data: kanalens skyddsutkast som stöds anger nu att datauppsättningssökning fungerar för alla kanaler där beslut är tillgängligt (kodbaserad upplevelse, e-post, push, SMS och innehållsbeslutsaktivitet under resor). Begränsad tillgänglighet och allmänna betaversioner har tagits bort från sidorna för beslutsregler, rangordningsformler och beslutsobjekt. [Läs mer](../experience-decisioning/aep-data-exd.md)

* Integreringssidan för externa system har uppdaterats med länkar till anpassade datakällor och anpassade åtgärder, och klargör att offresproxy tillhandahåller en statisk IP för utgående anrop från **anpassade åtgärder** till dina externa system. [Läs mer](../configuration/external-systems.md)

* Körningsdokumentationen för körning av resedagring har klargjorts: steghändelseattributen `inDryRun` och `dryRunID` dokumenterar nu att de returnerar `true`/instans-ID i körläge och `null` för test- eller direktresor. Vägledningen för att exkludera händelser i Dry-körningssteg i rapporteringsfrågor har uppdaterats i enlighet med detta. [Läs mer](../building-journeys/journey-dry-run.md)

* **Webbpush** är nu allmänt tillgänglig. Dokumentationen för push-meddelanden har omstrukturerats och uppdaterats i enlighet med detta (kom igång, design, skicka, skapa). [Läs mer](../push/get-started-push.md)

* Sidan för konfiguration av webb-push finns nu i dokumentationen. [Läs mer](../push/push-configuration-web.md)

* Dokumentationen om hur du använder fragment i beslut har uppdaterats: anteckningar har lagts till i avsnitten Fragment och Beslut och sidan Fragments in Decision policies har uppdaterats. [Läs mer](../experience-decisioning/fragments-decision-policies.md)

* Dokumentationen för SMS-webkroken har uppdaterats: Twilio-webbkrokinnehållet har tagits bort. [Läs mer](../sms/sms-webhook.md)

* Dokumentationen för **Konvertera bilder till innehållsmallar** har förbättrats med utökade skyddsutkast och rekommendationer, vanliga användningsexempel och tydligare vägledning för konvertering av bilddesign till redigerbara HTML-innehållsmallar. I den nämns också att du nu kan använda ett tema som indata för konverteringen. [Läs mer](../content-management/image-to-html.md)

* Dokumentationen för API för migrering av beslut har uppdaterats. [Läs mer](../experience-decisioning/decisioning-migration-api.md)

* Aktiviteten **Innehållsbeslut** är nu allmänt tillgänglig. Aktivitetssidan för innehållsbeslut har uppdaterats med ett avsnitt om beslutsdata som är tillgängliga i steghändelser. [Läs mer](../building-journeys/content-decision.md)

* Länkar till API-dokumentationen för lojalitetsutmaningar har lagts till i avsnittet för lojalitetsutmaningar (komma igång, skapa utmaningar, skapa uppgifter, få tillgång till lojalitetsutmaningar). [Läs mer](../loyalty-challenges/get-started.md)

* Den kanalinformation som stöds i dokumentationen för guiden Skapa kampanj har korrigerats. Sidorna Kom igång med kanaler och Vanliga frågor och svar om samordnade kampanjer har uppdaterats i enlighet med detta. [Läs mer](../campaigns/get-started-with-campaigns.md)

* Behörighetsdokumentationen har korrigerats med avseende på behörigheterna **Resehantering** och **Godkänn**. [Läs mer](../administration/ootb-permissions.md)

* Integreringsdokumentationen för AEM (Adobe Experience Manager) har uppdaterats med reviderat namnutrymme (AEM Dynamic Content och AEM fragments). [Läs mer](../integrations/aem-fragments.md)

* En ny exkluderingsorsak har lagts till i undantagslistan: **UnsubscribeLinkNotValid** (felkod 050081). Det här undantaget genereras när Ämneslängden för List-Unsubscribe mailTo är större än RFC-gränsen på 998 tecken. [Läs mer](../reports/exclusion-list.md)

* Hjälpfunktionens formatDate-dokumentation har förbättrats med en anteckning om att funktionen kräver en fälttyp för datum/tid (inte en sträng) och med flera exempel: formatera ett datum/tid-fält, konvertera en sträng till datum först, fullständigt datum med dagnamn, dynamiskt datum från systemtid och veckodagsformat inklusive gemener. [Läs mer](../personalization/functions/dates.md#format-date)

* E-postdokumentationen för textversionen har förbättrats med omfattande vägledning om användningsfall, inklusive beslutskriterier för när anpassad oformaterad text ska användas jämfört med automatisk synkronisering, praktiska exempel med verkliga scenarier och ett avsnitt med vanliga frågor och svar med vanliga frågor. [Läs mer](../email/text-version-email.md#when-to-use)

* Dokumentationen för e-postteman för Designer har uppdaterats med information om begränsningar för stöd av webbteckensnitt och vikten av reservteckensnitt. [Läs mer](../email/apply-email-themes.md#themes-guardrails)

* En begränsning har lagts till i hjälpdokumentationen för exekveringsmetadata för att klargöra att metadata inte fångas in för profiler som utesluts från åtgärden. [Läs mer](../personalization/functions/helpers.md#execution-metadata)

* Dokumentationen för kodbaserade implementeringsexempel har uppdaterats för att inkludera tokenfältet i propositionAction för korrekt spårning och attribuering i beslutet. [Läs mer](../code-based/code-based-implementation-samples.md#client-side-how)

* En anteckning har lagts till i dokumentationen för URL-spårning och för att avbeställa en lista för att klargöra att ordningen för URL-spårningsparametrar som läggs till i URL-adresser är slumpmässig och inte kan kontrolleras. [Läs mer](../email/url-tracking.md)

## Januari 2026 {#january-2026}

* Kontrollpanelens dokumentation om licensanvändning har klargjorts med uppdaterad vägledning om **Aktiverbara profiler**, inklusive definitionsinformation och felsökningsvägledning. [Läs mer](../audience/license-usage.md#what-is-engageable-profile)

* En anteckning har lagts till i e-postdokumentationen för Designer-teman för att förtydliga begränsningar för webbteckensnitt. [Läs mer](../email/apply-email-themes.md#themes-guardrails)

* Ett nytt skyddsavsnitt har lagts till för att dokumentera validering av nyttolaststorlek för resan, inklusive varnings- och feltrösklar samt vägledning om hur man optimerar resorna. [Läs mer](../start/guardrails.md#journey-payload-size)

* Dokumentationen för skyddsordningarna för beslut har uppdaterats så att den omfattar storleksbegränsningar för beslutsobjekt (1 kB för objekt som inkluderar attribut med högst 30 attribut). [Läs mer](../experience-decisioning/decisioning-guardrails.md)

* En anteckning har lagts till i dokumentationen för att skapa beslutsprinciper för att informera användare om att när en beslutspolicy väl har skapats kan det ta upp till 15 minuter att sprida ändringarna över alla dataområden, och upp till 30 minuter för Kanada. [Läs mer](../experience-decisioning/create-decision-policy.md#review)

* En anteckning har lagts till i fragmentdokumentationen som varnar om att när både knappetiketten och URL-adressen kan redigeras i ett fragment, loggar spårningsdatauppsättningen URL-värdet i stället för etikettvärdet. [Läs mer](../content-management/customizable-fragments.md#visual)

* Det finns nu en ny sida som beskriver fördelarna med att migrera från beslutshantering till beslut, inklusive information om kommande migreringsverktyg för API:er. [Läs mer](../experience-decisioning/migrate-to-decisioning.md)

* Ett skyddsutkast har lagts till för att förtydliga att uppslagsdatauppsättningar endast är tillgängliga för inkommande kantbaserad aktivering i den region där datauppsättningens sandlåda finns. [Läs mer](../data/lookup-aep-data.md#guidelines)

* Ett nytt avsnitt har lagts till i konfigurationsdokumentationen för Orchestrated-kampanjkanaler där det förklaras hur kontextattribut (t.ex. kampanj-ID, namn och åtgärdsinformation) ska användas i URL-spårningsparametrar för analyser och rapportering. [Läs mer](../orchestrated/channel-config.md#url-tracking)

* Dokumentationen för innehållsoptimering har omstrukturerats för att bli tydligare. Den huvudsakliga optimeringssidan har delats upp i fyra fokuserade delsidor: en startsida, en dedikerad sida för målinriktning, en för experimenterande och en annan för att kombinera båda metoderna. [Läs mer](../content-management/gs-message-optimization.md)

* Anteckningarna för begränsad tillgänglighet har tagits bort från tre färdvarningar (Resurs Published, Resney Finished och Custom Action Capping Triggered) eftersom dessa funktioner nu är allmänt tillgängliga. [Läs mer](../reports/alerts.md)

* Landningssidan Testa, validera och godkänn har förbättrats med nya avsnitt som testningskapacitet, vanliga frågor och svar, beslutsträd med navigeringslänkar och förbättrad terminologi med dokumentationslänkar. [Läs mer](../../rp_landing_pages/test-landing-page.md)

* Ett nytt avsnitt har lagts till i dokumentationen för personaliseringssyntaxen för att klargöra hur reserverade nyckelord ska användas i personaliseringsuttryck. Vissa PQL-nyckelord som `next`, `last` och `this` måste föregås av en bakterie när de används som fältnamn i XDM-schemat. [Läs mer](../personalization/personalization-syntax.md#reserved-keywords)

* Sidorna [Kom igång med kampanjer](../campaigns/get-started-with-campaigns.md) och [Hantera kampanjer](../campaigns/manage-campaigns.md) har omstrukturerats med förbättrad informationsarkitektur, inklusive ett omfattande arbetsflöde med typspecifika guider, förbättrade jämförelser av kampanjtyper och en konsoliderad statustabell.

* Landningssidan Journeys har gjorts om för att underlätta introduktionen med ett nytt 6-stegsarbetsflöde, förbättrade jämförelser av resetyper och förbättrad navigering genom hela dokumentationen. [Läs mer](../building-journeys/journey.md)

* Ett detaljerat avsnitt har lagts till för att hjälpa användare att generera Base64-kodade privata OpenSSH-nycklar för SFTP-autentisering när de konfigurerar filroutning för Direct Mail för att undvika anslutningsfel. [Läs mer](../direct-mail/direct-mail-configuration.md#ssh-key-generation)

* En anteckning har lagts till i dokumentationen för delegering av underdomäner för att informera användare om att tillåta 24-48 timmar för DNS-spridning innan de försöker delegera till Adobe. [Läs mer](../configuration/delegate-subdomain.md#set-up-subdomain)

## December 2025 {#december-2025}

* De anpassade målgrupperna för överföring för beslutsdokumentation har uppdaterats så att en nödvändig API-flagga för hämtning av anrikningsdata ingår. När du använder CSV-överförda målgrupper i offertbeslut måste du inkludera `"xdm:enrichedAudience": true` i din API-begäran och nyttolast för att hämta anrikningsattribut i svaret på erbjudandebeslutet. [Läs mer](../offers/custom-upload-decisioning.md#must-read)

* En anteckning har lagts till i korrekturutskicksdokumentationen för att klargöra att reglerna för frekvensbegränsning gäller för korrektur. Sidan innehåller nu ett&quot;Must-read&quot;-avsnitt med viktiga överväganden om hur frekvensbegränsning fungerar, begränsningar av spegelsidor och tillgänglighetsregler för resurser. [Läs mer](../content-management/proofs.md)

* En ny tabell över tillgängliga kommunikationskanaler har lagts till på sidan Kom igång med kanaler, som visar vilka kanaler som stöds över resor och kampanjer (åtgärdskampanjer, API-utlösta kampanjer och Orchestrated-kampanjer). [Läs mer](../channels/gs-channels.md#channels)

* En ny omfattande landningssida för spårning har skapats för att hjälpa användare att upptäcka och få tillgång till alla spårnings- och övervakningsfunktioner som är tillgängliga i Journey Optimizer. [Läs mer](../start/get-started-tracking.md)

* Sidan för hantering av avanmälan via e-post har förbättrats med detaljerad information om avanmälningsflödet och en förklaring av den förväntade ordningen för händelser för avanmälan på landningssidan. [Läs mer](../email/email-opt-out.md#send-message-unsubscribe-link)

* Dokumentationen för prenumerationslistan har uppdaterats för att innehålla information om kvalificeringskriterier för direktuppspelade segment. [Läs mer](../landing-pages/subscription-list.md#define-subscription-list)

* Det finns en ny leveransguide för IP-värmare som ger omfattande vägledning om rykte, förberedelser inför flight, övervakningsstatistik och bästa praxis för övergång från nollpublicering till framgångsrik placering i inkorgen. [Läs mer](../configuration/ip-warmup-deliverability-guide.md)

* En varning har lagts till på landningssidan och avsnittet om avanmälan via e-post för att klargöra att när du klickar på en avanmälningslänk öppnas endast landningssidan, men användarna måste skicka formuläret för att slutföra avanmälningsprocessen. [Läs mer](../landing-pages/lp-use-cases.md#configure-opt-out)

* Ett nytt bibliotek med användningsfall för resor finns nu tillgängligt, som innehåller en samling praktiska användningsfall, inklusive taktiska mönster (undertryckningslogik, personaliseringstekniker, strategier för att avsluta resor) och kompletta scenarier som omfattar arbetsflöden för marknadsföring och teknik. [Läs mer](../building-journeys/jo-use-cases.md)

* Det finns nu ett nytt användningsexempel som visar hur du konfigurerar en resa för att skicka e-post endast på veckodagar (måndag-fredag), med automatisk kö för helgposter som ska skickas på måndag vid en viss tidpunkt. [Läs mer](../building-journeys/weekday-email-uc.md)

* Nu finns en ny sida som förklarar Journey Optimizer beslutsmöjligheter, inklusive skillnaderna mellan nästa generations beslutsramverk och den färdiga beslutshanteringslösningen, och deras viktigaste fördelar när det gäller att leverera personaliserade erbjudanden i olika kanaler. [Läs mer](../experience-decisioning/gs-decision.md)

* Ett nytt avsnitt har lagts till i dokumentationen om målgruppsaktivering som förklarar hur målgruppstyper (t.ex. Customer Journey Analytics-målgrupper) som inte stöds i [!DNL Journey Optimizer] aktiveras genom att de kapslas in i en ny segmentdefinition i målportalen. [Läs mer](../audience/target-audiences.md#activation-non-supported)

* Ett nytt avsnitt har lagts till i dokumentationen för aktiviteten Vänta där det förklaras hur profiler som partades vid en aktiviteten Vänta i Läs målgruppsresor automatiskt uppdaterar sina attribut från UPS (Unified Profile Service). Detta klargör att profildata kan ändras under körningen efter en väntenod, vilket kan ge oväntade resultat om du förväntar dig konsekventa ögonblicksbildsdata under hela resan. [Läs mer](../building-journeys/wait-activity.md#profile-refresh)

* Ett varningsmeddelande har lagts till i avsnittet Sökvägsexperiment som varnar användare om att inte redigera metadata för ett sökvägsexperiment när det har publicerats, eftersom detta kommer att störa beräkningen och rapporteringen av experimentresultat. [Läs mer](../building-journeys/optimize.md#experimentation)

* En anteckning har lagts till i avsnittet Skapa en formulärförinställning för att ange kraven för direktuppspelningsanslutningar som ska visas i listrutan för val. [Läs mer](../landing-pages/lp-forms.md#create-form-preset)

* En ny sida finns nu i avsnittet Beslut om hur du konfigurerar datainsamling för att spåra visningar, klickningar och anpassade händelser. [Läs mer](../experience-decisioning/data-collection/schema-requirement.md)

* Genereringen av innehåll med AI-assistentdokumentation har omstrukturerats för att bli tydligare och mer användbar. De fem föregående kanalspecifika sidorna (e-post, push, SMS, webb och landningssida) har konsoliderats till tre generationsspecifika sidor: [Generera fullständigt innehåll](../content-management/generative-full-content.md), [Generera text](../content-management/generative-text.md) och [Generera bilder](../content-management/generative-image.md).

## November 2025 {#november-2025}

* Det finns nu en ny sida med frågor och svar om beslutsfattande som omfattar ämnen som appningsregler, konfiguration av AI-modell, trafikkrav och strategier för optimering av erbjudanden. [Läs mer](../experience-decisioning/decisioning-faq.md)

* Sidan Kom igång med e-postdesign har uppdaterats för att förtydliga hur du kommer åt e-postprogrammet Designer. [Läs mer](../email/get-started-email-design.md)

* Ett felsökningsavsnitt har lagts till på DMARC postsida för att åtgärda fördröjning av DNS-spridning. [Läs mer](../configuration/dmarc-record.md#troubleshooting)

* Sidan Arbeta med GenStudio for Performance Marketing har förbättrats med nya avsnitt som nyckelfunktioner, vanliga användningsområden, krav och vanliga frågor och svar. [Läs mer](../integrations/genstudio.md)

* En garanti för pseudonyma profiler med inkommande kanaler har lagts till på sidan Guardrails and constraints: Om oautentiserade besökare används för målgruppsanpassning ökar det totala antalet profiler som kan användas, så Adobe rekommenderar att du ställer in en TTL (Time-To-Live) för automatisk borttagning av profiler för att hantera de associerade kostnaderna. [Läs mer](../start/guardrails.md#profile-management-inbound)

* Två självstudiekurser om hur du konfigurerar Web SDK för beslut och kodbaserade upplevelser finns nu på exempelsidan för kodbaserade implementeringsmetoder. [Läs mer](../code-based/code-based-decisioning-implementations.md#tutorials)

* En anteckning har lagts till för att ange att resurser och bilder ska vara tillgängliga i upp till två år (730 dagar) från den första publiceringen och måste publiceras på nytt efter att publiceringen har upphört. [Läs mer](../content-management/proofs.md)

* Nu finns en omfattande guide för frågor om AI Assistant. Den här guiden visar hur ni skapar effektiva uppmaningar för att skapa varumärkesanpassat marknadsföringsmaterial med hög konverteringsgrad. Lär dig de bästa sätten att skriva marknadsföringsmål, använda varumärkesresurser och optimera innehåll för olika kanaler. [Läs mer](../content-management/ai-assistant-prompting-guide.md)

* En anteckning har lagts till i segmentdefinitionsdokumentationen för att klargöra att attributet `frequencyMap` inte stöds för användning i segmentdefinitioner och inte kan användas som en del av målgruppssegmenteringskriterierna. För frekvensbaserad målinriktning bör du överväga att använda regler för frekvensbegränsning enligt affärsreglerna. [Läs mer](../audience/creating-a-segment-definition.md)
* Ett nytt exempel som visar hur du använder anpassade åtgärdssvar i inbyggda kanaler har lagts till i dokumentationen för API-anropssvar. Exemplet visar hur du itererar över kapslade arrayer från anpassade åtgärdssvar med hjälp av Handlebars syntax i e-post-, push- och SMS-meddelanden. [Läs mer](../action/action-response.md#response-in-channels)

* Ett nytt avsnitt har lagts till i integreringsdokumentationen för Campaign v7/v8 som förklarar hur du uppdaterar befintliga anpassade åtgärder när slutpunkten för realtid (RT) ändras. Avsnittet innehåller stegvisa instruktioner för att uppdatera slutpunkts-URL:en, testa anslutningen och validera ändringarna innan du sparar. [Läs mer](../action/acc-action.md#update-action)

* Nya avsnitt om begränsningar och bästa praxis har lagts till i dokumentationen om visuella fragment för att varna användare om kapsling av fragment som innehåller dynamiskt innehåll i andra olåsta fragment med dynamiskt innehåll som inte stöds. Vägledningen innehåller felsökningssteg för problem med kompatibilitetsläge och rekommendationer för korrekt design av e-poststruktur. [Läs mer](../email/use-visual-fragments.md#fragment-dynamic-content)

* Ett felsökningsavsnitt har lagts till i resedokumentationen för live-rapportering för att hjälpa användarna att lösa problem med saknade rapporteringsdata. Avsnittet behandlar synkronisering av resenamn med rapportdatamängder, tidsangivelser för datauppdatering, verifiering av åtkomstbehörigheter och krav på resestatus. [Läs mer](../building-journeys/report-journey.md#troubleshooting-missing-data)

* Tre nya artiklar med vanliga frågor och svar har lagts till i resursdokumentationen som förklarar när tillgångar förfaller och livscykelhantering. Bland de ämnen som behandlas finns TTL-regeln (Time-To-Live) för AEM-mediefiler (730 dagar), hur du löser problem med brutna bilder på grund av att mediefilen har gått ut samt information om kommande förbättringar av logiken för att förfalla mediefiler. [Läs mer](../integrations/assets.md#faq-assets)

* Ett omfattande felsökningsavsnitt har lagts till i dokumentationen om aktiviteten Läs målgrupp för att åtgärda felmatchningar mellan beräknade och faktiska profiler som kommer in på resor. Avsnittet behandlar frågor om timing och dataspridning, datavalidering och övervakningstekniker samt bästa praxis, inklusive användningen av alternativet&quot;Utlösare efter grupputvärdering&quot;. [Läs mer](../building-journeys/read-audience.md#audience-count-mismatch)

* En anteckning har lagts till i dokumentationen för publikkvalificeringshändelser för att förtydliga fördröjning för direktuppspelningssegmentering (upp till 2 timmar) och rekommendera att en vänteaktivitet eller bufferttid läggs till för tidskänsliga resor. [Läs mer](../building-journeys/audience-qualification-events.md#streamed-speed-segment-qualification)

* Ett nytt avsnitt har lagts till i e-postsäkerhetsdokumenten som dokumenterar storleksgränsen på 2 MB för att skicka meddelanden, inklusive bästa praxis för att behålla skrivet innehåll under 1 MB för att möjliggöra serverdelsbearbetning av overheaddata. [Läs mer](../start/guardrails.md#message-content-size)

* Förbättrad dokumentation för alternativet Inkrementell läsning i Läs publikens aktiviteter för att förtydliga beroenden av ögonblicksbildtiming och 24-timmars look-back-begränsning, inklusive rekommendationer för att förhindra saknade profiler. [Läs mer](../building-journeys/read-audience.md)

* En anteckning lades till i datasetsökningsskyddsjournalerna för att ange att sökningar inte kan kopplas ihop. [Läs mer](../data/lookup-aep-data.md#guidelines)
...
* Det finns nu en ny omfattande guide om kriterier för inträde och utträde på resan, som bland annat innehåller bästa praxis, exempel från verkligheten och praktisk vägledning för hur man hanterar när profiler går in och ut på resorna i Adobe Journey Optimizer. [Läs mer](../building-journeys/entry-exit-criteria-guide.md)

* Det finns nu en ny sida som förklarar hur du itererar över kontextuella data i meddelanden. Den här guiden beskriver hur du använder Handlebars-syntax för att visa dynamiska listor från händelser, anpassade åtgärdssvar, dataset-sökningar och andra sammanhangskällor i din personalisering. [Läs mer](../personalization/iterate-contextual-data.md)

* Frågan för att identifiera ignorerade händelser under resor har korrigerats så att den innehåller korrekta filter för fel i segmentexportjobb, borttagning av dispatcher och ignorering av tillståndsdatorer. [Läs mer](../reports/query-examples.md#common-queries)

* Introduktionsmeningar har lagts till i alla 37 frågeexempel i frågeexempeldokumentationen för att ge bättre kontext och förklara vad varje fråga gör innan SQL-koden presenteras. Detta förbättrar användarförståelsen och ger tydligare vägledning om när varje fråga ska användas. [Läs mer](../reports/query-examples.md)

## Oktober 2025 {#october-2025}

* Nu kan du konvertera bilder till HTML-mallar med hjälp av bilden till HTML Converter. [Läs mer](../content-management/image-to-html.md)
...
* En lista med skyddsförslag lades till i avsnittet med information om hur fragment som kopplats till beslutsposter inom beslutspolicyer kan utnyttjas. [Läs mer](../experience-decisioning/use-decision-policy.md#fragments)
...
* Dokumentationen för Experimentation Accelerator har flyttats till en separat samling. [Läs mer](https://experienceleague.adobe.com/sv/docs/experimentation-accelerator/using/overview)
...
* Optimeringen på kampanjsidan uppdaterades för att återspegla det faktum att optimering nu även är tillgängligt på resorna. [Läs mer](../content-management/gs-message-optimization.md)

* En länk till självstudievideon med en beskrivning av hur man utnyttjar meddelandeoptimering i en kampanj lades till. [Läs mer](../content-management/gs-message-optimization.md)
...
* I avsnittet Beslutsfattande finns en vägledning som förklarar att godkända erbjudandeposter inte kan tas bort om de används i en samling eller ett beslut. Inkluderade steg för att ändra deras status till Utkast med alternativet **[!UICONTROL Undo approve]**. [Läs mer](../experience-decisioning/items.md#manage)
...
* API för beslutshantering för att lista personaliserade erbjudanden har uppdaterats med ett exempel för att utföra paginering om flera personaliserade erbjudanden saknas i svaret. [Läs mer](../offers/api-reference/offers-api/personalized-offers/offers-list.md)
...
* Den inbyggda sidan för kanalåtgärder har omstrukturerats. [Läs mer](../building-journeys/journey-action.md)
...
* En video om hur AI Assistant Content Generator genererar e-postmeddelanden har lagts till. [Läs mer](../content-management/generative-full-content.md#video)
