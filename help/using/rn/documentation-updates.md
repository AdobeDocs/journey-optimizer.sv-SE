...
* Ett skyddsutkast om målanpassning av pseudonyma profiler med inkommande kanaler har lagts till på sidan [Guardsändningar och begränsningar](../start/guardrails.md#profile-management-inbound). Detta skyddsutkast anger att målprofiler som inte är autentiserade eller ännu inte kända ökar det totala antalet profiler som kan användas och kan påverka din fakturering. Därför rekommenderar Adobe att du ställer in en TTL-inställning (Time-To-Live) för automatisk borttagning av profiler för att hantera ditt antal profiler och därmed sammanhängande kostnader.
