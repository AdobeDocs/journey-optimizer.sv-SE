...
1. Se till att du har rätt behörigheter för Journey Optimizer-kampanjer innan du startar, även om du bara tänker använda meddelanden i appen under resor. Kampanjbehörigheter krävs fortfarande. [Läs mer](../campaigns/get-started-with-campaigns.md#prerequisites).
...
>[!CAUTION]
>
>När du riktar in dig på pseudonyma profiler (oautentiserade besökare) med dina meddelanden i appen bör du överväga att ställa in en TTL (Time-To-Live) för automatisk borttagning av profiler för att hantera ditt engagerande profilantal och tillhörande kostnader. [Läs mer](../start/guardrails.md#profile-management-inbound)
...
* I [Adobe Experience Platform Data Collection](https://experienceleague.adobe.com/docs/experience-platform/edge/datastreams/overview.html?lang=sv-SE){target="_blank"} kontrollerar du att du har en datastream definierad, till exempel under tjänsten **[!UICONTROL Adobe Experience Platform]** som du har Adobe Experience Platform Edge och alternativet **[!UICONTROL Adobe Journey Optimizer]** aktiverat.
  ...
  >[!NOTE]
  >
  >Attributen `context.datastream` stöds för närvarande bara för webbkanalskampanjer i Journey Optimizer. Försök att använda `context.datastream` i meddelanden i appen resulterar i valideringsfel som `Invalid syntax Missing schema field: 'datastream`.
...
1. Välj den plattform som du vill definiera inställningarna för. På så sätt kan ni ange målappen för varje plattform och säkerställa enhetlig innehållsleverans på flera plattformar.
   ...
   >Om du vill begränsa leveransen av meddelanden i appen till en viss plattform måste du implementera enhetsspecifika regler under kundresan eller kampanjlogiken.
