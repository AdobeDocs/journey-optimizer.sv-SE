...
Om du har aktiverat **[!UICONTROL Enable List-Unsubscribe]** vad gäller rubriken för att avbeställa en lista rekommenderar vi att du aktiverar båda metoderna - **Mailto (avsluta prenumeration)** och **Avbeställ URL med ett klick**. Alla e-postklienter har inte stöd för HTTP-metoden. Funktionen för att avsluta prenumerationen på e-postlistan är till för att du ska kunna välja ett alternativ, och ditt avsändarrykte kan skyddas bättre och alla mottagare kan använda funktionen för att avbryta prenumerationen. [Läs mer](email-settings.md#list-unsubscribe)
