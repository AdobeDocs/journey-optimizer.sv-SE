...
## Spårningsparametrar som visar tomma platshållare i stängda resor {#tracking-parameters-closed-journeys}

Om spårnings-URL:er i skickade e-postmeddelanden innehåller tomma platshållare, till exempel `cid=em-acou-adob{}`, kan det tyda på att ett kontextfält som `context.system.source.actionId` inte kunde matchas. Detta inträffar vanligtvis när en resa stängs och inte har publicerats på nytt efter en relevant produktändring - endast återpublicerade resor fyller i dessa kontextfält korrekt i spårning av URL:er.

Lös detta genom att antingen publicera om resan ([skapa en ny version och publicera den](publish-journey.md#journey-create-new-version)) eller ta bort referensen till det påverkade kontextfältet från [URL-spårningsparametrarna](../email/url-tracking.md) i kanalkonfigurationen eller e-postinnehållet.
