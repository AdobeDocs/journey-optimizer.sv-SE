...
När du använder en affärshändelse (se [det här avsnittet](../event/about-events.md)) kan du i testläget utlösa en enskild testprofilentré under resan, simulera händelsen och skicka rätt profil-ID. Du måste godkänna händelseparametrarna och identifieraren för den testprofil som ska gå in på resan i testet. Du kan inte använda alternativet **[!UICONTROL Up to 100 profiles at once]** som finns för andra segmentbaserade resor. I testläge finns det inget kodläge tillgängligt för resor baserat på affärshändelser.
